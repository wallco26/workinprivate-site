...
Marketing website for **WorkInPrivate** — a private AI chatbot that runs open-source AI models locally (models are downloaded from Hugging Face). Hosted on GitHub Pages at `www.workinprivate.com`. Sells the product ($29.99 one-time) built in the sibling `workinprivate/` repo.
...
- Pricing: single product at $29.99 one-time (no modules, no subscriptions)
- Primary CTA: "Buy Now — $29.99" linking to `/pricing`
